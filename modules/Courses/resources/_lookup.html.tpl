{if $userCourses}
<div class="form-group alert alert-info p-3 mt-1">
    ...
    <!-- <form action="syllabus/courses" method="get" id="courseSelectForm"> -->
        <select class="form-control" name="courses" id="courseSelectLookup">
            ...
            {foreach $userCourses as $course}
                <option value="{$course->id}" {if $realSection->external_key == $course->id}selected{/if}>
                    {if $course->sectionNumber}[{$course->getShortName()}] {/if}{$course->title}
                </option>
            {/foreach}
        </select>
    <!-- </form> -->
